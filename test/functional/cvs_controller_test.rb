...
class CvsControllerTest < ActionController::TestCase
  ...
  test "should create cv" do
    assert_difference('Cv.count') do
      post :create, cv: { about: @cv.about, contact_email: @cv.contact_email, phone: @cv.phone, user_id: @cv.user_id }
    end
  ...
  test "should update cv" do
    put :update, id: @cv, cv: { about: @cv.about, contact_email: @cv.contact_email, phone: @cv.phone, user_id: @cv.user_id }
    assert_redirected_to cv_path(assigns(:cv))
  end
